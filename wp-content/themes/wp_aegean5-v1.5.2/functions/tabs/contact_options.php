...
<?php else: ?>
	...
	<fieldset class="set">
		<p class="guide"><?php echo sprintf(__('Select your booking form page, that you have created and assigned the "Booking Form" page template. This is to redirect properly when checking availability from any page on the site. If blank, the booking form will be hidden. The booking form e-mail address is where the e-mails will be send. If empty, the administrator\'s email address will be used (<em>%s</em>)', 'ci_theme'), get_option('admin_email')); ?></p>
		<fieldset>
