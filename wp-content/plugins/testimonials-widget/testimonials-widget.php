...
EOF;
	...
	public static function get_testimonials_html( $testimonials, $atts, $is_list = true, $widget_number = null ) {
		...
		foreach ( $testimonials as $testimonial ) {
			...
			$content = self::get_testimonial_html( $testimonial, $atts, $is_list, $widget_number );

			if ( $target )
				$content = links_add_target( $content, $target );
			$content  = apply_filters( 'testimonials_widget_testimonial_html', $content, $testimonial, $atts, $is_list, $widget_number );
			$html    .= $content;
			//$is_first = true; - this will remove the filteration of number of testimonial to be display
		}
	...
	public static function get_testimonial_html( $testimonial, $atts, $is_list = true, $is_first = false, $widget_number = null ) {
		...
		$div_open .= ' bubble" style="background: #dadada; border-color: #dadada;">';
	...
	public static function get_testimonials( $atts ) {
		...
		if(get_post_meta( get_the_ID(), 'testimony', true )){
			$cat_name = get_post_meta( get_the_ID(), 'testimony', true );
		}else{
			$cat_name = $category;
		}
